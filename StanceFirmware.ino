...
I2CReceiver i2c(MY_I2C_ADDRESS);
...
#ifdef MARC_SERIAL_PORT
    MARC_SERIAL_PORT.begin(MARC_SERIAL_BAUD_RATE);
    marcduinoSerial.setStream(&MARC_SERIAL_PORT);
#endif
...
MARCDUINO_ACTION(ChangeToTwoLegged, #TWOLEGS, ({
    if (stance.isThreeLegged())
    {
        // Let's get it started
        stance.changeToTwoLegged();
    }
    else if (stance.isTwoLegged())
    {
        // Already on two legs
    }
    else
...
MARCDUINO_ACTION(ChangeToThreeLegged, #THREELEGS, ({
    if (stance.isTwoLegged())
    {
        // Let's get it started
        stance.changeToThreeLegged();
    }
    else if (stance.isThreeLegged())
    {
